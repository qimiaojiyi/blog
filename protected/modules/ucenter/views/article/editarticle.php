...
<ul class="breadcrumb">
    <li><a href="/">首页</a> > </li>
    <li><a href="<?php echo __APP__?>/ucenter/article/type">文章分类</a>><a href="<?php echo __APP__?>/ucenter/article/articlelist?typeid=<?php echo $arcinfo['typeid']?>"><?php echo $breadcrumbs;?></a> > 修改文档</li>
...
<!--添加文章层-->
    <div class="m_typebox noborder">
        <form action="<?php echo __APP__?>/ucenter/article/editarticle" method="post" name="addform" enctype="multipart/form-data">
        <ul class="padding17">
            <li style="font-size: 14px;line-height: 25px;">
            <p>
                <input type="hidden" name="typeid" value="<?php echo $arcinfo['typeid'];?>">
                <input type="hidden" name="arcid" value="<?php echo $arcinfo['id'];?>">
                文章标题：
                <input name="title" type='text' value="<?php echo $arcinfo['title'];?>">
                &nbsp;&nbsp;
                定义属性：
                ...
                文章图片：
                <?php if($arcinfo['litpic']):?>
                <input type='hidden' name="litpic" value="<?php echo $arcinfo['litpic']?>">
                <div id="imagePreview" style="display:inline-block;cursor: pointer;width:100px;height:75px;border:1px solid #ccc;margin-bottom: 17px;border-radius: 3px;text-align: center;line-height: 75px;color:#ccc;font-size: 14px;"><img src="<?php echo $arcinfo['litpic'];?>" style="position: relative; width: 102px; height: 77px;"></div>
            ...
            <p>
                文章内容：
                <script id="editor" name="ucontent" type="text/plain" style="width:90%;height:300px;margin-left: 74px;"><?php echo $arcinfo['content']?></script>
