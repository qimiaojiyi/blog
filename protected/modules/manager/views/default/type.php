...
/*    .content{
    ...
    .childimg{
        width:16px;margin-right:5px; cursor:pointer;
    }
</style>
<ul class="breadcrumb">
    <li><a href="/">首页</a> > </li>
    ...
    <li style="position: absolute;right:17px;top:-3px;">
        <div class="btn-toolbar">
            <div class="btn-group">
                <a class="btn btn-mini" id="addtypebtn" role="button" href="#addtype" data-toggle="modal">添加栏目</a>
    ...
    <div class="m_typebox">
        <ul>
            <li style="height:25px;font-size: 14px;line-height: 25px;" id="<?php echo $type['id']?>">
                ...
                <span class="opentype m_fright"><a href="<?php echo __APP__?>/manager/default/arclist?typeid=<?php echo $type['id'] .'?'.rand(0,99)?>">打开</a></span>
...
<script>
    ...
    $('#addtypebtn').click(function(){
        ...
        var url = '<?php echo __APP__?>/manager/default/type';
        $.ajax({
            type: 'POST',
            url : url,
            data : {'flag':'api'},
    ...
    $('.childimg').click(function(){
            var self = $(this).parent('span').parent('li'),
                pid = self.attr('id'),
                url = '<?php echo __APP__?>/manager/default/childtype';
            if(self.hasClass('on')){
                self.removeClass('on');
